...
class ChongqingTravelPage extends StatelessWidget {
  const ChongqingTravelPage({super.key, this.locale = 'zh'});
  ...
  Widget build(BuildContext context) {
    return Theme(
      ...
      child: TravelShell(initialLocale: locale),
    );
  }
}
...
class TravelShell extends StatefulWidget {
  const TravelShell({super.key, this.initialLocale = 'zh'});
...
class _TravelShellState extends State<TravelShell> {
  ...
  void initState() {
    super.initState();
    // 同步主页传过来的语言到全局
    if (AppLocale.instance.lang != widget.initialLocale) {
      AppLocale.instance.setLang(widget.initialLocale);
    }
  }

  AppLanguage _languageFromLocale() =>
      AppLocale.instance.isKo ? AppLanguage.ko : AppLanguage.zh;
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        final language = _languageFromLocale();
        return Scaffold(
          body: SafeArea(
            child: Column(
              children: [
                ...
                Expanded(
                  child: IndexedStack(
                    ...
                    children: [
                      HomePage(
                        language: language,
                        onToggleLanguage: () => AppLocale.instance.toggle(),
                      ),
                      AttractionsPage(language: language),
                      FoodPage(language: language),
